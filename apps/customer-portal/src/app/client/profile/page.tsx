...
import { Building2, Mail, Lock, User, Phone, KeyRound } from 'lucide-react';
...
export default function ClientProfile() {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    setSaveMsg(null);
    try {
      const res = await api.patch('/client/auth/profile', { fullName, phone });
      const data = res.data?.data || res.data;
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/client/auth/change-password', { oldPassword, newPassword });
      if (res.data?.ok) {
    ...
    <div className="flex-1 flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-lg relative z-10 space-y-6">
        <motion.div
          ...
          className="glass-panel w-full p-8 rounded-2xl"
        >
          ...
          {loading ? (
              ...
              <div className="grid grid-cols-1 gap-4 mb-6 p-4 bg-white/5 rounded-xl border border-white/10">
                ...
                <div>
                  ...
                  <p className="text-white font-medium flex items-center gap-2">
                    ...
                    {String(profile.role || profile.roleName || 'CUSTOMER_CLIENT').replace(/_/g, ' ')}
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveProfile} className="space-y-4">
                ...
                {saveMsg && (
                  <div className={`p-3 rounded-lg text-sm text-center font-medium ${
                    saveMsg.includes('successfully') ? 'bg-emerald-500/20 border border-emerald-500/50 text-emerald-400' : 'bg-rose-500/20 border border-rose-500/50 text-rose-400'
                  }`}>
                    {saveMsg}
                  </div>
                )}
            ...
            <div className="text-rose-400 text-center py-8">Failed to load profile.</div>
          )}
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            {passwordMsg && (
              <div className={`p-3 rounded-lg text-sm text-center font-medium ${
                passwordMsg.includes('successfully') ? 'bg-emerald-500/20 border border-emerald-500/50 text-emerald-400' : 'bg-rose-500/20 border border-rose-500/50 text-rose-400'
              }`}>
                {passwordMsg}
              </div>
            )}
