...
const logger = new Logger('Logistics-Bootstrap');
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('SmartLogi Logistics Service')
    .setDescription('Logistics, Fleet, and Trip Management API for SmartLogi SaaS Platform')
    .setVersion('1.0')
    .addBearerAuth()
    .addTag('trips', 'Trip management endpoints')
    .addTag('vehicles', 'Vehicle fleet endpoints')
    .addTag('drivers', 'Driver management endpoints')
    .addTag('logistics', 'Logistics operations endpoints')
    .addTag('finance', 'Finance and COD endpoints')
    .addTag('ai', 'AI management endpoints')
    .build();
  ...
  logger.log(`Logistics Service is running on port ${port}`);
