...
export class AuditLogInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const resourceType = originalUrl.split('/')[3]?.toUpperCase() || 'SYSTEM';
...
export class TenantSchemaInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    // Set schema context from JWT user for tenant users
    // This interceptor runs after guards, so req.user is available
    if (user?.schemaName) {
